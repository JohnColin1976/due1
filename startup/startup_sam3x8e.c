...
extern uint32_t _siramfunc; // start of init values for .ramfunc in FLASH
...
void Reset_Handler(void) {
  ...
  // 4) CMSIS system init (clocks)
  SystemInit();

  // 5) Vector table base (important if any remap/boot ROM differences)
  SCB->VTOR = (uint32_t)g_pfnVectors;

  // 6) C runtime init (newlib) — constructors/init arrays like SES
  __libc_init_array();
